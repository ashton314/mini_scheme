...
Load syntactic analyzer implementation:
  ...
  perl scheme.pl [-quiet] [-no-init] [-no-stats] [-echo-file <filename>]

Load metacircular evaluator implementation:

  perl metacircular.pl [-quiet] [-no-init] [-no-stats]
...
There are (so far) four command line options for this interpreter:
